...
async function onHandleSubmit() {
    ...
    if( errorDetail.isError === false) {
        const fetchApi = await metodePembayaranAPI.create(formsInput);

        if( fetchApi.statusCode === 201 ) {
            toastSuccess('Metode pembayaran berhasil ditambahkan');
            await router.push('/admin/master/metode-pembayaran');
        } else {
...
<template>
    <section class="form-section">
        <div class="form-header-title">Tambah Metode Pembayaran</div>
        ...
        <div class="uk-form-stacked form-section-input">
            <form @submit.prevent="onHandleSubmit()">
                ...
                <div class="uk-margin">
                    <router-link :to="{ name: 'list-metode-pembayaran' }" class="uk-button uk-button-default uk-margin-small-right button button-default">
                        Kembali
                    </router-link>
